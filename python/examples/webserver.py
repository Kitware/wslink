...
if not os.path.exists(clientDir) or not os.path.exists(
    os.path.join(clientDir, "index.html")
):
    print(
        "Example client hasn't been built, please run 'npm run build:example' in ../js "
    )
...
wslink_server = create_webserver(config)
